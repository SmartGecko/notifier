...
var setupAmazonSes = function () {
    if (!validConfig()) {
        var errorMsg = 'missing server access key, secret or region, please update config.transport.ses section';
        logger.log(errorMsg);
        throw new Error(errorMsg);
    }

    var service = new aws.SES(_.extend({apiVersion: '2010-12-01'}, config.transport.ses));

    return {
        send: send
    };

    function send(name, context, message, callback) {
        if (!validOptions()) {
        ...
        emailTemplates(templatesDir, function (err, template) {
            ...
            template(name, context, function (err, html, text) {
                if (err)
                    return callback(err);
                ...
                if (text)
                    params.Message.Text.Html = {
                        Data: text
                    };

                try {
                    service.sendEmail(params, callback);
                } catch (e) {
                    logger.error(e);
                    callback(e);
                }
    ...
    function validConfig() {
        return config.transport.ses &&
            (config.transport.ses.accessKeyId && config.transport.ses.secretAccessKey && config.transport.ses.region);
    }
};
